...
public class VRMoveController : MonoBehaviour
{
    ...
    float CurrentTargetSpeed = 6f;

	public GB_playerScript1 GB;
	GameObject p1;
	...
	private void Start ()
	{
		GB = GetComponent<GB_playerScript1> ();
	...
	private void FixedUpdate ()
	{
		...
		if ((Mathf.Abs (input.x) > float.Epsilon || Mathf.Abs (input.y) > float.Epsilon)) {
			...
			if (GB.isTouching1 == true || GB.isTouching2 == true || GB.isTouching3 == true || GB.isTouching4 == true) {
				CurrentTargetSpeed = 5f;
			} else {
				CurrentTargetSpeed = 6f;
			}
